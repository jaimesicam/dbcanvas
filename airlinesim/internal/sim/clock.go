...
// days out becomes checkable in six real minutes, so a ten-minute demo sees several
// complete reservation lifecycles. Same rate as Hotel Sim, for the same reason.
const simRate = 720.0

// SimClock is an accelerated clock anchored at a (wallEpoch, simEpoch) pair. Reading
// it is cheap (no locking beyond a read lock); re-anchoring happens only at startup
// and Reset.
...
// Anchor re-anchors the clock: simulated time becomes `sim` right now, and advances
// at simRate from this instant forward. Used at startup (from a persisted checkpoint
// or a fresh Reset baseline) — never mid-run.
...
// used on Reset() so dates look plausible and every run is byte-for-byte repeatable
// in its relative structure.
