...
// WebSocket live-event bridge. Nothing here touches PostgreSQL directly except
...
func (h *Handler) Routes() http.Handler {
	...
	mux.HandleFunc("GET /api/locations/{id}", h.handleLocationDetail)
	mux.HandleFunc("GET /api/vehicles", h.handleVehicles)
...
// PostgreSQL itself is unreachable — this is also how the web interface's own
// "connection problem" banner ultimately gets its signal, via GET /api/state's
// error field using the same Ping.
func (h *Handler) handleHealthz(w http.ResponseWriter, r *http.Request) {
	if err := h.Store.Ping(r.Context()); err != nil {
		http.Error(w, "postgresql unreachable: "+err.Error(), http.StatusServiceUnavailable)
		return
	}
...
func (h *Handler) handleLocationDetail(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	detail, ok := h.Engine.LocationDetail(r.Context(), id)
	if !ok {
...
func (h *Handler) handleVehicles(w http.ResponseWriter, r *http.Request) {
	...
	rows, total, err := h.Engine.VehiclePage(r.Context(), r.URL.Query().Get("search"), r.URL.Query().Get("status"), limit, offset)
	if err != nil {
		http.Error(w, "query failed: "+err.Error(), http.StatusInternalServerError)
		return
	}
	writeJSON(w, map[string]any{"vehicles": rows, "total": total, "limit": limit, "offset": offset})
}

func (h *Handler) handleReservationDetail(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	locationID := r.URL.Query().Get("locationId")
	pickupDate := r.URL.Query().Get("pickupDate")
	detail, ok := h.Engine.ReservationDetail(r.Context(), id, locationID, pickupDate)
	if !ok {
