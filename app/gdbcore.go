...
//
// And once a core is open the command at the top of this comment is handed back filled in —
// gdbRecipe, below — with the node, the binary, the core and the search path this installation
// actually resolved, so the session can be continued in a terminal or pasted into a ticket.
...
// gdbRecipe is the command line that reproduces this session in a terminal.
//
// The page is a better way to read a core than gdb's own prompt, right up to the moment it is not:
// a command this tool does not offer, a colleague who wants the session on their own screen, a
// note in a ticket that has to say what was actually run. That last one is the reason this is
// generated rather than written down anywhere — the recipe in the header of this file is the shape
// of the answer, but the real one carries the paths and the search path THIS node ended up with,
// and a recipe you retype from memory is a recipe that quietly reads different libraries.
//
// It is deliberately the same command DBCanvas runs (gdbStartScript), minus the parts that exist
// only because a machine is on the other end:
//
//   - `--interpreter=mi3` and the `stty`, which are the machine interface itself, and
//   - `set confirm off`, which is right for a program driving gdb and wrong for a person, who
//     should be asked before something is thrown away.
//
// Everything that decides what the backtrace SAYS is kept, because a recipe that produces a
// different answer from the page is worse than none: sysroot, solib-search-path, the debug-file
// directory, the source directories, and `backtrace past-main/past-entry`, which is what keeps the
// bottom of a stack-exhaustion core from being cut off.
func gdbRecipe(container, binary, core, sysroot, solib string, srcDirs []string) string {
	if binary == "" || core == "" {
		return ""
	}
	var b strings.Builder
	if container != "" {
		// The mounts, the symbols and the source only exist inside the node, so the line has to
		// start there. -it because gdb's prompt is interactive.
		b.WriteString("docker exec -it " + gdbShellArg(container) + " ")
	}
	b.WriteString("gdb -nx \\\n")
	for _, ex := range gdbRecipeSettings(sysroot, solib, srcDirs) {
		b.WriteString("  -ex " + strconv.Quote(ex) + " \\\n")
	}
	b.WriteString("  " + gdbShellArg(binary) + " " + gdbShellArg(core) + "\n")
	return b.String()
}

// gdbRecipeSettings is the -ex list, in the order gdbStartScript uses it.
func gdbRecipeSettings(sysroot, solib string, srcDirs []string) []string {
	out := []string{
		"set auto-load off",
	...
	if len(srcDirs) > 0 {
		out = append(out, "directory "+strings.Join(srcDirs, " "))
	}
	// The one command everybody types next, and the reason most people opened gdb at all.
	return append(out, "thread apply all bt")
}

// gdbShellArg quotes a path for a shell only when it needs it. Every path here is a mount point or
// a file name from one, so in practice nothing is quoted — and a line full of quotes nobody needs
// is a line people edit before they paste, which defeats the point of handing them an exact one.
func gdbShellArg(s string) string {
	if s == "" {
		return "''"
	}
	for _, r := range s {
...
func (a *App) gdbNodeIssues(n designNode, st Stack) []issue {
	...
	if st.Backend == BackendVagrant && a.vagrant != nil {
		return []issue{{Level: "error", Message: name + " is set up for core-dump analysis, which needs bind mounts — those are available on the Docker backend only"}}
	}
	...
	for label, p := range map[string]string{"core-dump directory": n.GDBCoreDir, "library directory": n.GDBLibDir} {
		if _, err := gdbCleanHostPath(p); err != nil {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("%s: the %s %s", name, label, err)})
		}
	}
	if cores, libs := path.Clean(n.GDBCoreDir), path.Clean(n.GDBLibDir); cores != "" && cores == libs {
		out = append(out, issue{Level: "warning", Message: name + " mounts the same directory as both the core dumps and the libraries — that works, but the libraries are usually a separate copy of the crashed host's /lib64"})
	}
	...
	if !sliceHas(gdbProducts, product) {
		out = append(out, issue{Level: "error", Message: name + " asks for debug symbols for an unknown product " + strconv.Quote(product)})
		return out
	}
	if strings.TrimSpace(n.GDBMajor) == "" {
		out = append(out, issue{Level: "error", Message: name + " needs the " + gdbProductLabel(product) + " major series the core came from — that is what selects the debug-symbol packages"})
		return out
	}
	...
	if entry == nil {
		out = append(out, issue{Level: "error", Message: fmt.Sprintf("%s runs %s %s, which %s publishes no packages for — set the node's OS to the one the crashed server ran",
			name, n.OS, n.OSVersion, gdbProductLabel(product))})
		return out
	}
	minors := entry.Versions[strings.TrimSpace(n.GDBMajor)]
	if len(minors) == 0 {
		out = append(out, issue{Level: "error", Message: fmt.Sprintf("%s asks for %s %s on %s %s, which the catalog has no builds of — pick one from the list, or run `make versions`",
			name, gdbProductLabel(product), n.GDBMajor, n.OS, n.OSVersion)})
		return out
	}
	if v := strings.TrimSpace(n.GDBVersion); v != "" && !sliceHas(minors, v) {
		out = append(out, issue{Level: "error", Message: fmt.Sprintf("%s asks for %s %s, which is not in the catalog for %s %s — pick one from the list, or run `make versions`",
...
// gdbSourceMax caps one source read. Generated code aside (sql_yacc.cc and friends), the largest
// file in the server is a few hundred KiB; anything past this is not source anybody is reading a
// crash in, and a cap has to exist because this crosses a websocket into a browser.
const gdbSourceMax = 2 << 20

// gdbReadSource returns the WHOLE file, and whether the cap above cut it short.
//
// The whole file, not a window around the frame's line, because reading a crash is not reading one
// line: you scroll up to see what the function was given, down to see what it does with it, and
// out to the function above. A window ends exactly where the question you are asking starts —
// twenty-nine lines of fts0que.cc says what the line is and nothing about how it got there — and a
// pane that stops scrolling mid-file reads as broken rather than as bounded.
//
// It is affordable because of what these files are: source on a node that is one exec away, a few
// hundred KiB at worst, read once per frame you click. The Operator Debugger reads the operator's
// source the same way and for the same reason.
//
// Read off the node with head rather than through gdb's `list`: the file is right there, and gdb's
// list output is a format to parse rather than content to show.
func (a *App) gdbReadSource(ctx context.Context, id, file string) ([]string, bool, error) {
	if err := gdbSafeSourcePath(file); err != nil {
		return nil, false, err
	}
	// One byte past the cap, so a file that exactly fills it can be told from one that does not.
	res, err := a.engCtx(ctx).Exec(ctx, id, []string{"sh", "-c",
		fmt.Sprintf("head -c %d %s", gdbSourceMax+1, shellQuote(file))}, nil)
	if err != nil {
		return nil, false, fmt.Errorf("read %s: %w", file, err)
	}
	if res.Code != 0 {
		return nil, false, fmt.Errorf("read %s: %s", file, strings.TrimSpace(res.Stderr))
	}
	out := res.Stdout
	truncated := len(out) > gdbSourceMax
	if truncated {
		out = out[:gdbSourceMax]
		// The cut lands mid-line; drop that line rather than showing half of one as if it were
		// the code. A file this size has no last line worth the confusion.
	...
	if len(lines) == 1 && lines[0] == "" {
		return nil, false, fmt.Errorf("%s is not on this node — install the debugsource package for this version", file)
	}
