...
// a systemd OS image — no product installed on top, no PMM monitoring. Never
// published to the host: it's reached from inside a lab's Ubuntu VNC desktop's own
// browser, at hotelsim.<domain>:8089 on the stack's internal network.
...
func (a *App) provisionHotelSim(st Stack, n designNode, doc designDoc) {
	...
	cfg := hotelSimConfig{Image: hotelSimImage, Hostname: host, FQDN: fqdn}
	if !ok {
