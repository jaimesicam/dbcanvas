...
func dirAuthIssues(n designNode, dirNodes map[string]string) []issue {
	...
	if n.LdapAuth {
		if _, ok := dirNodes[n.LdapDirNodeID]; !ok {
			out = append(out, issue{Level: "error", Message: nodeKindLabel(n.Type) + " node " + n.Label + " has LDAP integration enabled but no directory is selected — add an Intranet or Samba AD DC node and pick it"})
		}
	}
	if n.KerberosAuth {
		hasSamba := false
		...
		if !hasSamba {
			out = append(out, issue{Level: "error", Message: nodeKindLabel(n.Type) + " node " + n.Label + " has Kerberos enabled, which requires a Samba AD DC node — add a Samba AD DC node or turn off Kerberos"})
		}
	}
	return out
}
