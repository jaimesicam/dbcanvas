...
export const HELP = {
  ...
  lsMode: 'Linked resolves the line you drew to a database on the canvas. Manual takes the connection typed here and can point anywhere, including outside the stack.',
  lsEngine: 'Which relational engine. JDBC only: MongoDB and Valkey have no driver here — use a Stock Market Sim node for those.',
  lsDriver: 'Which JDBC driver to load. Two ship for the MySQL family so the same server can be driven by each in turn, which is how a driver problem is told apart from a server problem. Swappable on the dashboard while it runs.',
  lsHost: 'Hostname or address. A comma-separated list becomes a multi-host JDBC URL (failover for Connector/J, targetServerType=primary for pgJDBC).',
  lsPort: 'Leave blank for the engine default — 3306 for MySQL, 5432 for PostgreSQL.',
  lsUser: 'The database user the pool authenticates as.',
  lsPassword: 'Stored with the deployment secrets, never shown in the node profile.',
  lsDatabase: 'Schema/database the ledger tables live in. Defaults to "ledgersim".',
  lsTLS: 'Intent, not a literal property: each driver spells it differently and the image maps it, explaining what it chose. MariaDB Connector/J has no opportunistic mode, so "prefer" becomes sslMode=disable there.',
  lsParams: 'Extra driver properties appended to the URL, written as a=1&b=2. They override anything the image derived.',
  lsJdbcUrl: 'A complete JDBC URL used verbatim, beating every other field. Works in linked mode too — that is how a node linked to a cluster can be aimed at one member of it.',
  lsPoolMode: 'Pooled keeps connections open and hands them out (HikariCP). Direct opens one per transaction and closes it after — which is how a great many real applications connect, from cron jobs to serverless handlers. Running the identical workload both ways against one server is what turns "use a connection pool" from advice into a number. Switchable on the dashboard as well, so one node can measure both.',
  lsPoolMax: "HikariCP's maximumPoolSize. Setting it below the worker count makes the pool a queue, which is a useful thing to demonstrate and a confusing thing to hit by accident.",
  lsThreads: 'How many concurrent workers drive transactions. Each borrows a connection per transaction, which is what makes the pool metrics mean something.',
  lsIsolation: 'The isolation level each transaction runs at. Blank means the driver default, which differs between MySQL and PostgreSQL — itself worth being able to see.',
  lsRevenueShards: 'Every order credits one of these accounts, so this is the contention knob: 1 serialises every transaction in the system on a single row, 8 spreads it.',
  lsDeadlockShare: 'The share of transactions that take their two row locks in the opposite order, producing real engine-detected deadlocks rather than a description of them.',
  benchThreads:
  ...
  uiPalette: 'The Infrastructure Library — every node type you can put on the canvas, grouped by category. Click one to add it. Right-clicking the canvas offers the same catalog as a menu, and drops the node where you clicked.',
  uiInsertTemplate:
  ...
  mcaCredentials: 'Create the two MClusterAdmin accounts on this MongoDB when it deploys, so the panel does not have to authenticate as the root admin user. ' +
    ...
    'Both deliberately exclude root and clusterAdmin, so the panel cannot drop a database. Both are created with SCRAM-SHA-256 credentials, and this also ' +
    'names SCRAM-SHA-256 in the server\'s authenticationMechanisms — without that the server refuses them with an error about the mechanism that reads like a ' +
    'wrong password. The passwords are set on the MClusterAdmin node, not here. Ticking this after the database is deployed needs a redeploy of it.',
  mcaAdminPassword: 'The password this stack uses for the MClusterAdmin accounts. It lives on the panel because that is where you log in from, and every ' +
    'MongoDB in the stack that adds the credentials uses this same pair — a lab with three databases should not need three passwords for one account. ' +
    'Leave it empty for the default (MCLUSTERADMIN_PASSWORD / MCLUSTERADMIN_RO_PASSWORD in .env). Changing it needs a redeploy of the databases that carry ' +
    'the account, not only of this node.',
  mcaAccounts: 'The two accounts a MongoDB in this stack creates for this panel when its "Add MClusterAdmin credentials" box is ticked: madmin for full ' +
    'administration, madmin-ro for the monitoring dashboards. Copy one into the panel\'s own connect box, with the MongoDB node\'s name as the host — the ' +
    'panel is configured through its UI, so the URI is the one thing DBCanvas cannot fill in for you.',
  mcaViewOnly: 'Start the panel with every write disabled — no balancer toggle, no replica-set reconfigure, no kill-op, no index drop, no user management. ' +
    'This is the panel refusing, which is worth having even when the MongoDB account is read-only too: the button is not there to click, rather than there and ' +
    'answered with an authorization error.',
  maxTabs:
    'How many pages the tabbed main window will hold open at once. Raise it if you work across many ' +
    'pages and keep hitting the limit; lower it on a machine where the browser is already short of ' +
    'memory, because a hidden tab is still a mounted page — it keeps its scroll position, its ' +
...
const NODE_BLURB = {
  ...
  ledgersim: 'An order/payment ledger driven over JDBC with a HikariCP pool — the simulator for client-side questions.',
...
export const MENU_HELP = {
  ...
  downloadMongoDiag: 'Download this node\'s diagnostic.data and its mongod.log (mongos.log on a router) as one .tar.gz, holding a single ' +
    '<hostname>_log_diagnostic_data directory — so three members\' bundles unpack side by side instead of over each other. FTDC is what the server was doing, ' +
    'written once a second with no configuration; the log is what happened to it. Read them in DBCanvas\'s own FTDC Summary page, in a Big Hole node, or send ' +
    'the archive to somebody. Both stream, so a multi-gigabyte log costs nothing in memory on the way past, and DBCanvas finds the paths itself: a mongos keeps ' +
