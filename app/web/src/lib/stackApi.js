...
export function k3dApi(id, fid) {
  ...
  return {
    ...
    // pt-k8s-debug-collector. Keyed to the frame because the capture is of the
    // whole cluster, not of the k3s node it happens to be taken through.
    collectStatus: () => request('GET', `${base}/collect`),
...
// mongoDownloadURL builds the link for a MongoDB node's diagnostic bundle: its
// diagnostic.data and its log, in one .tar.gz. A URL rather than a fetch, for the
// same reason the file manager uses one — a plain navigation hands a multi-gigabyte
// archive to the browser's own download machinery instead of buffering it in JS.
// What goes in it, and the name of the directory it unpacks to, are the server's
// business (app/mongodl.go); nothing here knows where mongod keeps its log.
