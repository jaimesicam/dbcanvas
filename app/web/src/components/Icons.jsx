...
export const Icon = {
  ...
  // Database Stacks. It used to borrow Grid — four squares in a 2x2 — which at 18px in a
  // sidebar is the Dashboard's four panels with the corners rounded differently, and the two
  // sit six rows apart. A stack is layers, so this is layers: three plates in perspective,
  // which shares no silhouette with anything else in the set.
  Stacks: (p) => (
    <Svg {...p}>
      <path d="M12 3 3 7.6l9 4.6 9-4.6L12 3Z" />
      <path d="M3.2 12.2 12 16.7l8.8-4.5" />
      <path d="M3.2 16.6 12 21l8.8-4.4" />
    </Svg>
  ),
  // Kubernetes: the helm wheel — a heptagon, an inner hub and four spokes. Operator Summary
  // reads a cluster-dump and had a bug on it, borrowed from the Operator Debugger next to it;
  // a bug is what the DEBUGGER is for, and the summary is a report about Kubernetes.
  Kubernetes: (p) => (
    <Svg {...p}>
      <polygon points="12 3 19.04 6.39 20.77 14 15.91 20.11 8.09 20.11 3.23 14 4.96 6.39" />
      <circle cx="12" cy="12" r="3" />
      <path d="M12 3v6M12 15v5.1M20.77 14l-5.9-1.1M3.23 14l5.9-1.1" />
    </Svg>
  ),
  // Core Dump Analyzer: a memory chip with its contents coming out of it, which is what a core
  // dump is. The gem it replaced (Mineral) was a picture of the word "core".
  //
  // Pins on the top and bottom edges only. A chip with pins on all four sides is a body with
  // legs down both sides at 18px, and the row above this one in the sidebar is the Operator
  // Debugger's beetle.
  CoreDump: (p) => (
    <Svg {...p}>
      <rect x="5.5" y="5.5" width="13" height="13" rx="2" />
      <path d="M9.5 5.5V3M14.5 5.5V3M9.5 21v-2.5M14.5 21v-2.5" />
      <path d="M12 8.4v6.6M9.4 12.5 12 15.2l2.6-2.7" />
    </Svg>
  ),
  Grid: (p) => (
    <Svg {...p}>
