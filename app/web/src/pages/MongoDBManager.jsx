      ...
      {tab === 'creds' && (
        <div className="space-y-2">
          ...
          {[
            ...
            // The MClusterAdmin accounts, when this database was asked to create
            // them. They are here as well as on the panel node because this is
            // where somebody looks when the panel says authentication failed —
            // and an account whose password is written down nowhere is an account
            // nobody can use.
            ...(sec.mcaUser ? [
              { k: 'MClusterAdmin user (full)', v: sec.mcaUser },
              { k: 'MClusterAdmin password', v: sec.mcaPassword, secret: true },
            ] : []),
            ...(sec.mcaReadonlyUser ? [
              { k: 'MClusterAdmin user (read-only)', v: sec.mcaReadonlyUser },
              { k: 'MClusterAdmin read-only password', v: sec.mcaReadonlyPassword, secret: true },
            ] : []),
          ].map((r) => (
            <div key={r.k}>
