...
function Overview({ cfg, dep, onDeleteNode }) {
  return (
    <div className="space-y-2 text-sm">
      ...
      <KV k="Spock node" help={DEP_HELP['Spock node']} v={cfg.nodeName} mono />
      <KV k="Topology" help={DEP_HELP.Topology} v="Active-active (multi-master)" />
      <KV k="Mesh peers" help={DEP_HELP['Mesh peers']} v={(cfg.members || []).length ? `${(cfg.members || []).length} node(s)` : '—'} />
      <KV k="Database" help={DEP_HELP.Database} v={cfg.database} mono />
      <KV k="FQDN" help={DEP_HELP.FQDN} v={cfg.fqdn} mono />
      <KV k="PostgreSQL" help={DEP_HELP.PostgreSQL} v={cfg.pgVersion || cfg.pgMajor} mono />
      <KV k="Spock" help={DEP_HELP.Spock} v={cfg.spockRef ? `source @ ${cfg.spockRef}` : 'source'} mono />
