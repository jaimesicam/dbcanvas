...
// one row tall. It used to also print the type blurb and the OS/version line, which
// is what a canvas is worst at: three lines of prose in 212px wrapped, clipped at
// the card's bottom edge, and turned a diagram into a wall of half-sentences. Those
// details moved into the card's tooltip, where they have room to be readable.
const NODE_H = 56
const SNAP = 26
...
export const NODE_TYPES = {
  ...
  // (built from source at a pinned commit; see images/bighole.Dockerfile). Runs its
  // own image, so it carries no os/osVersion/arch.
  //
  // ports:false, unlike every other tool node here: it draws no association line
  // because it talks to nothing at all. You drag a diagnostic.data folder onto the
  // page; there is no database for it to point at.
  bighole: {
    label: 'Big Hole',
    slug: 'bighole',
    sub: 'MongoDB FTDC viewer — in the browser',
    color: '#10b981',
    icon: 'Monitor',
    singleton: false,
    ports: false,
  ...
  // it drives — but this panel talks to nothing at deploy time, so a connector on it
  // offered a line that carried a hint and no wiring. What replaces it: if the stack
  // has exactly one MongoDB, DBCanvas prepares that cluster's connection string
  // (mcaTarget in app/mclusteradmin.go); with several, the panel hands over the
  // madmin credentials and you type the host.
  mclusteradmin: {
    label: 'MClusterAdmin',
    slug: 'mclusteradmin',
    sub: 'MongoDB administration panel',
    color: '#10b981',
    icon: 'Sliders',
    singleton: false,
    ports: false,
...
// the role and version in the tooltip.
const PXC_NODE_H = 48
...
// NodeStatus is the status half of a node card, and it is a DOT for the state every
// node is in most of the time.
//
// "running" as a word cost about sixty of the card's 212 pixels and pushed the name
// into an ellipsis — "mcluster…", "stocksim-…" — which is the wrong trade: the name
// is the thing you are looking for, and "running" on a green pill is the least
// surprising sentence in the app. So running is a dot, and the states that are
// actually worth reading keep their word: an error, a stop, a node still pending.
// Provisioning keeps its ring, which carries a number a word cannot.
//
// The state is in the card's tooltip either way, so nothing is only a colour.
export function NodeStatus({ dep }) {
  if (!dep) return null
  ...
  return <Badge tone={DEPLOY_TONE[dep.state] || 'muted'}>{dep.state}</Badge>
}

// nodeCardTip is what a node card no longer prints: the type, what it is, and what
// it was built from. It is the tooltip on every card, and it exists because the card
// itself is now one row — an icon, a name and a status.
//
// Exported so the render smoke test can assert a card still ANSWERS for itself: the
// risk in trimming a card down is that the detail is not moved but lost.
export function nodeCardTip(n, def, dep, arch) {
  const built = deployedLabel(n.type, dep)
  return (
    <span className="block space-y-0.5">
      <span className="block font-semibold">{n.label}</span>
      <span className="block text-muted">{def.label}{def.sub ? ` — ${def.sub}` : ''}</span>
...
// memberCardTip is the same for a cluster member, whose card is smaller still. The
// role is the part worth reading — arbitrator, mongos, config server, shard member,
// primary or secondary — and it is what the card used to squeeze into 10px text.
export function memberCardTip(f, n, dep, sub, built, arch) {
  return (
    <span className="block space-y-0.5">
      <span className="block font-semibold">{n.label}</span>
...
// The Infrastructure Library holds 40 entries across 14 categories — far more than
...
// menuEntriesFor shapes the library catalog into context-menu entries: one per
// category, flattened to a direct action when the category holds a single item,
// because a submenu with one row in it is a wasted hop. Deliberately no
// recently-used block — the menu is opened at a point on the canvas and closed
// again in one gesture, so a list that reorders itself between openings moves the
// row you were reaching for. The docked library keeps its recents, where the panel
// stays put and the list is worth scanning. `toAction` turns a catalog entry into
// a menu action; kept a parameter so this stays pure and the smoke suite can
// assert the shape without a canvas to click on.
export function menuEntriesFor(groups, toAction) {
  const out = []
  for (const g of groups) {
    if (g.items.length === 1) {
      out.push(toAction(g.items[0]))
      continue
    }
    const items = g.items.map(toAction)
    // A category whose every entry is unavailable is itself unavailable. Opening
    // it to find nothing but greyed rows tells you nothing the closed row could
    // not, and before the Intranet goes on that is *every* category — which would
    // make a first right-click eleven submenus of dead ends. The reason comes from
    // the children when they agree on one, which they do in the cases that matter:
    // "Add an Intranet node first" before anything exists, "Locked while deploying"
    // during a deploy.
    const disabled = items.every((it) => it.disabled)
    const reasons = new Set(items.map((it) => it.help))
    out.push({
      label: g.title,
      items,
      disabled,
      help: disabled
        ? (reasons.size === 1 ? [...reasons][0] : 'Nothing in this category can be added right now')
        : undefined,
    })
  }
  return out
...
// error that named it.
//
// It exists because of the split in `make images`: that command builds the operating
// system bases and the Intranet, and every optional image on top of them is another
// (`make extra-images`), so a stack can be designed against an image nobody has built
// yet — including the Intranet, whose build `make images` reports and tolerates
// rather than failing a first run over. Sending somebody to a terminal in
// a checkout is a poor answer when DBCanvas is already talking to the daemon that
// would do the build — so an admin can do it here, and everyone else is told the
// target that does it.
//
// Progress is polled rather than streamed: the build runs in a container of its own
// (app/extraimages.go) and its output arrives when the step finishes, so there is
// nothing to stream until then. What matters while waiting is that it is still
// going, and how long it has been.
export function BuildImageRow({ id, isAdmin, onBuilt }) {
  ...
  const load = useCallback(async () => {
    try {
      const r = await imageApi.list()
      const found = (r.images || []).find((x) => x.id === id) || null
      setImg(found)
      return found
    } catch (e) {
      setErr(e.message)
  ...
  // While a build is running, poll — and when it stops, re-validate so the error
  // this row sits under goes away by itself.
  const building = !!img?.build?.running
  useEffect(() => {
    if (!building) return undefined
    const t = setInterval(async () => {
      const next = await load()
      if (next && !next.build?.running) {
        clearInterval(t)
        if (next.present && onBuilt) onBuilt()
      }
    }, 2000)
    return () => clearInterval(t)
  }, [building, load, onBuilt])

  if (!img) return null

  const build = img.build
  const start = async () => {
    setErr(''); setAsked(true)
    try {
      await imageApi.build(id)
      await load()
    } catch (e) {
      setErr(e.message)
    }
  ...
  return (
    <div className="mt-1 space-y-1">
      {!img.buildable ? (
        <div className="text-xs text-muted">{img.why}</div>
      ) : !isAdmin ? (
        <div className="text-xs text-muted">
          An administrator can build it from here, or run <code className="rounded bg-surface2 px-1">make {img.make}</code> in a DBCanvas checkout.
        </div>
      ) : build?.running ? (
        <div className="flex items-center gap-2 text-xs text-muted">
          <span className="h-3 w-3 animate-spin rounded-full border-2 border-surface2 border-t-primary" />
          Building {img.tag} — this reaches out to npm, GitHub or a Percona repo, so it can take a few minutes.
        </div>
      ) : (
        <div className="flex flex-wrap items-center gap-2">
          <Button size="sm" variant="outline" disabled={asked} onClick={start}>
            <Icon.Play size={14} /> Build {img.label}
          </Button>
          <span className="text-xs text-muted">
            On this Docker daemon. The equivalent of <code className="rounded bg-surface2 px-1">make {img.make}</code>.
          </span>
        </div>
      )}
      {build?.error && (
        <div className="space-y-1">
          <div className="text-xs text-danger">Build failed: {build.error}</div>
          {build.log?.length > 0 && (
            <pre className="max-h-40 overflow-auto rounded-lg bg-surface2 p-2 text-[10px] leading-snug text-muted">{build.log.slice(-40).join('\n')}</pre>
          )}
        </div>
      )}
...
function StackEditor({ stackId, templates = [], onTemplatesChanged, onBack }) {
  ...
  const { settings, system } = useSettings() // per-user: how the library is offered; instance-wide: upload ceiling, SSH forwarding, experimental features
  // 'menu' (the default) drops the docked column and adds nodes from the canvas
  // right-click instead. The add menu works in both modes — this only decides
  // whether the Infrastructure Library column is there as well.
  const libraryColumn = settings.nodeLibrary === 'docked'

  const wrapRef = useRef(null)
  const dragRef = useRef(null)
  // Where the next node or cluster should land, in canvas coordinates. The add
  // menu sets it to the point that was right-clicked so a node appears under the
  // cursor; the Infrastructure Library leaves it null, and everything falls back
  // to the fixed viewport offset it has always used. Cleared after every add, so
  // one right-click never positions the *next* thing added from somewhere else.
  const dropAt = useRef(null)
  ...
  // Right-click on empty canvas: offer the whole library, and remember where the
  // pointer was so whatever gets picked lands *there*. Node and frame right-clicks
  // stop propagation before this, so it only ever fires on bare canvas.
  function openAddMenu(e) {
    ...
    setAddMenu({
      x: e.clientX,
      y: e.clientY,
      at: { x: (e.clientX - r.left - v.x) / v.z, y: (e.clientY - r.top - v.y) / v.z },
    })
  }
  ...
  // createFlow adds an association edge between two endpoints, and reports whether it
  // made one. from→to is a storage convention, not a claim about which way traffic
  // runs: the canvas draws these lines without an arrowhead and every resolver that
  // reads them — backendFrameForProxySQL, haproxyClusterFrames, each sim's target
  // lookup — walks them undirected. What the two ends *do* carry is cardinality: a
  ...
  function tryConnect(e1, e2) {
    ...
    // another ProxySQL still resolves the upstream backend — and the single-incoming
    // guard could disable the true answer, leaving only the backwards one enabled.
    // So just link them, taking whichever orientation the cardinality allows.
    if (k1 === 'proxysql' && k2 === 'proxysql') {
      if (!createFlow(e1, e2)) createFlow(e2, e1)
      return
    }
  ...
  function addPXCCluster() {
    ...
    const fid = uid('frame')
    const { x: fx, y: fy } = placeAt(200, 200)
    const frame = {
  ...
  // placeAt returns where to drop a new node or cluster: the point the add menu
  // was opened at, or the fixed offset from the top-left of the viewport that the
  // palette has always used. dx/dy are that fallback, in screen pixels.
  function placeAt(dx, dy) {
    const at = dropAt.current
    if (at) return at
    return { x: (-refs.current.view.x + dx) / refs.current.view.z, y: (-refs.current.view.y + dy) / refs.current.view.z }
  }
  ...
  function nodeMenuActions(id) {
    ...
    if (dep) {
      ...
      if (dep.state === 'running') {
        ...
        // knows (app/mongodl.go), so this asks it.
        if (node && ['psm', 'psmrs', 'psmdb'].includes(node.type)) {
          const isMongos = node.role === 'mongos'
          actions.push({
            label: `Download diagnostic.data with ${isMongos ? 'mongos' : 'mongod'}.log`,
            help: MENU_HELP.downloadMongoDiag,
            // A plain navigation, so the browser's own download machinery takes a
            // multi-gigabyte archive rather than a fetch buffering it.
            fn: () => { window.location.href = mongoDownloadURL(stack.id, id) },
          })
        }
  ...
  // visibleGroups drops whatever this installation has not switched on (see
  // lib/experimental.js), before anything reads the catalog: the docked library,
  // its search, and the canvas right-click menu all build from paletteGroups, so
  // filtering here is what makes one flag cover all three. A group left with no
  // items goes too, rather than leaving an empty heading.
  const paletteGroups = visibleGroups([
    { title: 'Core', items: [
    ...
    { title: 'MySQL (Community)', items: [
      ...
      { label: 'MySQL InnoDB / GR', type: 'mysqlceinnodb', onClick: addMySQLCEInnoDBCluster },
    ] },
    { title: 'MariaDB', items: [
    ...
    { title: 'MongoDB', items: [
      ...
      { label: 'MClusterAdmin', type: 'mclusteradmin', onClick: () => addNode('mclusteradmin') },
      { label: 'Big Hole (FTDC viewer)', type: 'bighole', onClick: () => addNode('bighole') },
    ] },
    ...
    { title: 'App Simulators', items: [
      ...
      // labelled: the category itself is finished work and no longer says
  ...
  // (the reason recents exist is to be one click away), then one entry per
  // category — flattened to a direct action when the category holds a single item,
  // since a submenu with one row in it is a wasted hop.
  const addMenuActions = (at) => {
    const add = (it) => {
      const { disabled, reason } = entryState(it)
      return {
        label: it.label,
        help: reason,
        disabled,
        // dropAt is what turns "add" into "add here" — placeAt reads it, every
        // builder goes through placeAt, and it is cleared straight after so a
        // later add from the palette lands at the usual offset.
        fn: () => {
          dropAt.current = at
          // The menu shows no recents, but it still feeds them: the list belongs to
          // the library, not to one front end, so switching to the docked column
          // finds a recents list of what this account has actually been adding.
          remember(it.label)
          try { it.onClick() } finally { dropAt.current = null }
        },
      }
    }
    return menuEntriesFor(paletteGroups, add)
  }
  ...
  // Whether a library entry can be used right now, and the one line that explains
  // it either way. Shared by the two front ends onto the same catalog — the docked
  // palette and the canvas add menu — so an entry can never be offered by one and
  // refused by the other.
  const entryState = (it) => ({
    disabled: it.off || (it.type !== 'intranet' && !hasIntranet) || deploying,
    reason: deploying
  ...
  return (
    <div className="flex h-[78vh] gap-4">
              ...
              {issues.map((it, i) => (
                <li key={i} className="flex items-start gap-2 text-sm">
                  ...
                  <span className="min-w-0 flex-1 text-fg">
                    {it.message}
                    {/* A missing image DBCanvas can build itself. The server put the
                        catalogue id on the issue (see missingImageIssue), so this is
                        not the front end guessing from the message. */}
                    {it.image && <BuildImageRow id={it.image} isAdmin={isAdmin} onBuilt={runValidate} />}
                  </span>
                </li>
              ))}
            </ul>
                      ...
                      <div key={n.id} className="group absolute"
                        ...
                        >
                          ...
                          {/* Name and status. The role and the version are in the
                              tooltip (memberCardTip): they were 9 and 10px here,
                              which is small enough to be decoration rather than
                              information. */}
                          <Hint
      ...
      {addMenu && (
        <ContextMenu menu={addMenu} onClose={() => setAddMenu(null)} actions={addMenuActions(addMenu.at)} />
      )}
      ...
      {gdb && (
        <>
          ...
          <Button size="sm" className="w-full" onClick={() => {
            sendHandoff('dbcanvas.gdbTarget', `${stackId}/${dep.nodeId}`)
            location.hash = 'core-dump'
          }}>
            <Icon.CoreDump size={15} /> Open analyzer
          </Button>
        </>
      )}
      <Button variant="danger" size="sm" className="w-full" onClick={onDeleteNode}>
...
// BigHoleLink is the way into a deployed Big Hole, and it is NOT SimDashboardLink.
//
// The app needs a secure context — HTTPS or localhost — for the private on-disk
// storage and workers it decodes a capture into. Opened as http://<host>:<port> it
// cannot keep a capture and fails partway through ingest, which reads like a broken
// decoder rather than a deployment mistake. SimDashboardLink builds its URL from
// location.hostname, which is right for a dashboard and wrong here: on a DBCanvas
// browsed at http://192.168.x.y:8080 it would hand over the one address that does
// not work.
//
// So the link is always localhost, and when this browser is somewhere else the row
// says so and gives the tunnel that fixes it.
export function BigHoleLink({ port }) {
  if (!port) return null
  const onLocalhost = typeof location === 'undefined' ||
    location.hostname === 'localhost' || location.hostname === '127.0.0.1'
  const url = `http://localhost:${port}/`
  const tunnel = `ssh -N -L ${port}:127.0.0.1:${port} <you>@<docker-host>`
  return (
    <div className="space-y-1.5">
      <a href={url} target="_blank" rel="noreferrer"
        className="flex items-center justify-center gap-2 rounded-lg border border-primary/40 bg-primary/10 px-3 py-2 text-sm font-medium text-primary hover:bg-primary/15">
        <Icon.External size={15} /> Open Big Hole
      </a>
      {!onLocalhost && (
        <div className="space-y-1 rounded-lg border border-warning/30 bg-warning/10 px-2.5 py-1.5 text-xs text-warning">
          <div>
            You are not browsing DBCanvas on localhost, and this viewer only works there — a
            browser gives a page the on-disk storage it decodes a capture into over HTTPS or
            localhost, nowhere else. Forward the port and open it as localhost:
          </div>
          <div className="flex items-center gap-1 rounded border bg-bg px-2 py-1">
            <span className="min-w-0 flex-1 truncate font-mono text-[11px] text-fg">{tunnel}</span>
            <CopyBtn text={tunnel} />
          </div>
        </div>
      )}
...
// BigHoleForm edits a (not-yet-deployed) Big Hole node. There is nothing to
// configure — no OS, no version, no association, no credentials — because the app
// talks to nothing: the only field is the label, and the rest is what to expect.
export function BigHoleForm({ node: n, patchNode, deleteNode, dep }) {
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold">Big Hole</span>
        {dep && <Badge tone={DEPLOY_TONE[dep.state] || 'muted'}>{dep.state}</Badge>}
      </div>
      <p className="text-xs text-muted">
        A MongoDB FTDC viewer that runs entirely in your browser. Drag a mongod&apos;s{' '}
        <span className="font-mono">diagnostic.data</span> folder — or a whole support tarball —
        onto the page and it decodes and charts it. Nothing is uploaded and there is no backend;
        this node is the built app behind nginx, published to a host port.
      </p>
      <div className="rounded-lg border border-warning/30 bg-warning/10 px-2.5 py-1.5 text-xs text-warning">
        Open it on <span className="font-mono">localhost</span>. Browsers only give a page the
        private on-disk storage it needs over HTTPS or localhost, so reaching it by host address
        leaves it unable to keep a capture — it fails partway through reading one. The node&apos;s
        panel gives you the port-forward line if you are not on the Docker host.
      </div>
      <p className="text-xs text-muted">
        For a capture from a node in this stack, take <span className="font-mono">diagnostic.data</span>{' '}
        from that node&apos;s File Manager first. DBCanvas&apos;s own <span className="font-medium">FTDC
        Summary</span> page reads the same files and answers a different question — verdicts and
        findings rather than every metric charted.
      </p>
...
// BigHoleManager shows a deployed Big Hole: the way in, and the commit it was built
// from — which is its only version, the project having no tags.
export function BigHoleManager({ dep, onDeleteNode }) {
  const cfg = dep?.config || {}
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
      ...
      <BigHoleLink port={cfg.httpPort} />
      <p className="text-xs text-muted">
        Drag a <span className="font-mono">diagnostic.data</span> folder or a support tarball onto
        the page. Nothing leaves your browser — there is no backend to send it to.
      </p>
      <div className="space-y-2 rounded-lg bg-surface2 px-3 py-2 text-sm">
        <InfoRow label="Internal URL" help={HELP.depInternalURL}><span className="font-mono text-xs">http://{cfg.fqdn || cfg.hostname}</span></InfoRow>
        <InfoRow label="Image" help={HELP.depImage}><span className="font-mono text-xs">{cfg.image || 'dbcanvas-bighole'}</span></InfoRow>
        <InfoRow label="Built from" help={HELP.bigHoleRef}><span className="font-mono text-xs">{(cfg.ref || '').slice(0, 12) || '—'}</span></InfoRow>
      </div>
      <Button variant="danger" size="sm" className="w-full" onClick={onDeleteNode}>
        <Icon.Trash size={16} /> Delete node
      </Button>
...
// Mirrors mcaDefaultPasswords() in app/mongodb.go — shown as the placeholder so an
// empty field says what the password will actually be rather than looking unset.
export const MCA_DEFAULT_ADMIN_PW = 'madmin_password'
export const MCA_DEFAULT_RO_PW = 'madmin_ro_password'

// MClusterAdminForm edits a (not-yet-deployed) MClusterAdmin node.
//
// No association line and nothing to point it at: the panel is configured through
// its own UI, so the connection URI is typed in there by whoever opens it. What this
// form owns is the pair of passwords the accounts get, and whether the panel starts
// read-only.
export function MClusterAdminForm({ node: n, patchNode, deleteNode, dep, deployed }) {
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold">MClusterAdmin</span>
        {dep && <Badge tone={DEPLOY_TONE[dep.state] || 'muted'}>{dep.state}</Badge>}
      </div>
      <p className="text-xs text-muted">
        A MongoDB administration panel: topology and replica-set status, sharding and the balancer,
        current operations, slow queries with explain, indexes, profiling, users and roles, oplog
        stats. Its web UI is published to the host once deployed — no VNC desktop needed.
      </p>
      <p className="text-xs text-muted">
        You enter the connection URI in the panel itself, which is why this node draws no
        association lines. Tick <span className="font-medium">Add MClusterAdmin credentials</span>{' '}
        on a MongoDB in this stack and it creates <span className="font-mono">madmin</span> and{' '}
        <span className="font-mono">madmin-ro</span> with the passwords below — then the host is
      ...
      {/* The passwords live on the panel, not on each database: this is the node you
          log in from, so it is the one place to look them up, and every MongoDB in
          the stack that adds the credentials uses this pair. */}
      <Field label="madmin password" help={HELP.mcaAdminPassword}
        hint={`Full administration. Empty = ${MCA_DEFAULT_ADMIN_PW}.`}>
        <input className={`${inputCls} ${deployed ? 'opacity-70' : ''}`} disabled={deployed}
          placeholder={MCA_DEFAULT_ADMIN_PW} value={n.mcaAdminPassword || ''}
          onChange={(e) => patchNode(n.id, { mcaAdminPassword: e.target.value })} />
      </Field>
      <Field label="madmin-ro password" help={HELP.mcaAdminPassword}
        hint={`Read-only. Empty = ${MCA_DEFAULT_RO_PW}.`}>
        <input className={`${inputCls} ${deployed ? 'opacity-70' : ''}`} disabled={deployed}
          placeholder={MCA_DEFAULT_RO_PW} value={n.mcaReadonlyPassword || ''}
          onChange={(e) => patchNode(n.id, { mcaReadonlyPassword: e.target.value })} />
      </Field>

      {/* A checkbox rather than a Toggle: every other option on this canvas is one,
          and Toggle is not even imported here. */}
      <label className={`flex items-center gap-2 text-sm ${deployed ? 'opacity-70' : ''}`}>
        <input type="checkbox" checked={!!n.viewOnly} disabled={deployed}
          onChange={(e) => patchNode(n.id, { viewOnly: e.target.checked })} />
        <span>Read-only panel (--view-only)</span><Help text={HELP.mcaViewOnly} />
      </label>
      {!!n.viewOnly && (
        <p className="text-xs text-muted">
          Sign in as <span className="font-mono">madmin-ro</span> to match it, and MongoDB refuses
          the writes the panel is already not offering.
        </p>
      )}
...
// MClusterAdminManager shows a deployed panel: the way in, and the two accounts to
// sign in with. No connection string — the panel takes the URI itself, so what
// DBCanvas can usefully hand over is the credentials, masked, and copyable while
// still masked (which is how they are actually used).
export function MClusterAdminManager({ dep, onDeleteNode }) {
  const cfg = dep?.config || {}
  const sec = dep?.secrets || {}
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {cfg.viewOnly && <Badge tone="muted">read-only</Badge>}
          <Badge tone={DEPLOY_TONE[dep.state] || 'muted'}>{dep.state}</Badge>
        </div>
      </div>
      ...
      <div className="space-y-2 rounded-lg bg-surface2 px-3 py-2 text-sm">
        <InfoRow label="madmin" help={HELP.mcaAccounts}><SecretInline value={sec.adminPassword} /></InfoRow>
        <InfoRow label="madmin-ro" help={HELP.mcaAccounts}><SecretInline value={sec.readonlyPassword} /></InfoRow>
      </div>
      <p className="text-xs text-muted">
        Connect from the panel with{' '}
        <span className="break-all font-mono">mongodb://madmin:&lt;password&gt;@&lt;host&gt;:27017/?authSource=admin</span>{' '}
        — a MongoDB in this stack has those accounts if you ticked{' '}
        <span className="font-medium">Add MClusterAdmin credentials</span> on it, and{' '}
        <span className="font-mono">&lt;host&gt;</span> is that node&apos;s name on the canvas (a
        replica set takes its members, comma-separated, plus <span className="font-mono">&amp;replicaSet=</span>).
      </p>

      <div className="space-y-2 rounded-lg bg-surface2 px-3 py-2 text-sm">
        <InfoRow label="Internal URL" help={HELP.depInternalURL}><span className="font-mono text-xs">http://{cfg.fqdn || cfg.hostname}:8787</span></InfoRow>
        <InfoRow label="Version" help={HELP.depVersion}><span className="font-mono text-xs">{cfg.version || '—'}</span></InfoRow>
      </div>
      <Button variant="danger" size="sm" className="w-full" onClick={onDeleteNode}>
        <Icon.Trash size={16} /> Delete node
      </Button>
...
// replica-set primary, or on the sharded cluster's config replica set, which is
// where its cluster-wide users live.
//
// Takes a value and an onChange rather than a node or a frame, because the three
// callers patch two different things.
//
// The roles are MClusterAdmin's own least-privilege sets, not ours — see mcaRolesJS
// in app/mongodb.go, where they are listed with the panel feature each exists for.
// The passwords are not here either: they belong to the panel node, so a stack with
// three MongoDBs does not end up with three passwords for the same account.
function MCACredentialsOption({ value, onChange, deployed }) {
  return (
    <div className="space-y-1">
      <label className={`flex items-center gap-2 text-sm ${deployed ? 'opacity-70' : ''}`}>
        <input type="checkbox" checked={!!value} disabled={deployed}
          onChange={(e) => onChange(e.target.checked)} />
        <span>Add MClusterAdmin credentials</span><Help text={HELP.mcaCredentials} />
      </label>
      {!!value && (
        <p className="text-xs text-muted">
          Creates <span className="font-mono">madmin</span> (full administration) and{' '}
          <span className="font-mono">madmin-ro</span> (read-only) with SCRAM-SHA-256, and enables
          that mechanism on this server. Passwords come from the MClusterAdmin node.
        </p>
      )}
    </div>
  )
...
function PSMStandaloneForm({ node: n, nodes, patchNode, deleteNode, dep, deployed }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <MCACredentialsOption value={n.mcaCredentials} deployed={deployed}
        onChange={(v) => patchNode(n.id, { mcaCredentials: v })} />
...
// MenuItem is one clickable row of a ContextMenu. A disabled action still renders,
// carrying its reason in the tooltip — "Add an Intranet node first" is the whole
// first lesson of the designer, and it only teaches anything if you can see the
// thing you cannot use yet.
//
...
// part is the one that matters — this used to clamp with a flat
...
// submenuPos places a submenu beside the row that opened it, in viewport coords.
// Opens to the right; flips to the left when the panel would run off that edge;
...
function Body({ selected, stackId, nodes, edges, frames, depByNode, patchNode, patchFrame, patchEdge, deleteNode, deleteEdge, deleteFrame, rebuildMongoCluster }) {
  ...
  if (selected.kind === 'node') {
    ...
    // Big Hole node — the browser-only MongoDB FTDC viewer.
    if (n.type === 'bighole') {
      if (dep && dep.state === 'running') {
        return <BigHoleManager dep={dep} onDeleteNode={() => deleteNode(n.id)} />
      }
      return <BigHoleForm node={n} patchNode={patchNode} deleteNode={deleteNode} dep={dep} />
    }
    // MClusterAdmin node — the MongoDB administration panel.
    if (n.type === 'mclusteradmin') {
      if (dep && dep.state === 'running') {
        return <MClusterAdminManager dep={dep} onDeleteNode={() => deleteNode(n.id)} />
      }
  ...
  // them undirected — so an arrow here would assert an order that means nothing.
  return (
    <div className="space-y-3">
      <div className="text-sm font-semibold">Association link</div>
