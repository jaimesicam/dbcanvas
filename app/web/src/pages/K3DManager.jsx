...
import { K8sCollectorCard } from '../components/Diagnostics.jsx'
...
kubectl get svc -n ${ns}`} />
      ...
      {tab === 'diag' && (
        frame
          ? <K8sCollectorCard stackId={stackId} frameId={frame.id} operator={cfg.operator} />
          : <div className="rounded-lg bg-surface2 px-3 py-2 text-xs text-muted">This node is not part of a Kubernetes frame.</div>
      )}
      ...
      {listening && (
        <div className="flex items-center justify-between gap-3 rounded-lg border bg-surface2 px-3 py-2">
          ...
          <Button size="sm" onClick={() => {
            sendHandoff('dbcanvas.debugTarget', `${stackId}/${frameId}`)
