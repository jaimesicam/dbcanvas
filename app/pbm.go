...
func pbmFrameIssues(f designFrame, doc designDoc) []issue {
	...
	if f.SeaweedFSNodeID == "" {
		return []issue{{Level: "error", Message: "PS MongoDB cluster " + f.Label + " has PBM backup enabled but no SeaweedFS node selected"}}
	}
	...
	return []issue{{Level: "error", Message: "PS MongoDB cluster " + f.Label + ": the selected PBM SeaweedFS node is not in the design"}}
}
