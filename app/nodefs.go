...
func (a *App) handleFSDownload(w http.ResponseWriter, r *http.Request) {
	...
	a.streamNodePaths(w, r, dep, paths, "")
}

// streamNodePaths is the download itself, split out of the handler so a caller that
// already knows what it wants can reuse it — the MongoDB log and diagnostic.data
// menu items (mongodl.go) are the same download of a path this app chose rather
// than one a user browsed to. `as` overrides the downloaded name; empty keeps the
// basename, which is what the file manager wants.
//
// One regular file streams raw; anything else (a directory, or several paths) is
// archived on the way out. Nothing is buffered: a 4 GB log and a hundred-file
// diagnostic.data both cost the same nothing in memory.
func (a *App) streamNodePaths(w http.ResponseWriter, r *http.Request, dep Deployment, paths []string, as string) {
	...
	if as != "" {
		name = strings.TrimSuffix(as, ".tar.gz")
	}
	a.streamNodeArchive(w, r, dep, paths, "", name)
}

// streamNodeArchive is the .tar.gz half of streamNodePaths, split out for the
// caller that always wants an archive and wants everything inside one directory:
// `root`, when set, is a folder every entry is written under, so unpacking the
// download leaves one named directory rather than loose files in the current one.
// That is what the MongoDB diagnostic bundle (mongodl.go) needs and what a person
// receiving it expects. `name` is the download's own name, without the .tar.gz.
func (a *App) streamNodeArchive(w http.ResponseWriter, r *http.Request, dep Deployment, paths []string, root, name string) {
	eng := a.engCtx(r.Context())
	...
	defer tw.Close()
	if root != "" {
		// The directory itself, so it exists even for an unpacker that does not
		// create parents, and so it carries a sane mode rather than the first
		// file's.
		if err := tw.WriteHeader(&tar.Header{
			Name: root + "/", Typeflag: tar.TypeDir, Mode: 0o755, ModTime: time.Now(),
		}); err != nil {
			return
		}
	}
	for _, p := range paths {
		if err := copyArchiveInto(r.Context(), eng, dep.ContainerID, p, root, tw); err != nil {
...
// several selections become one archive without any of it touching disk. `root`,
// when set, is prefixed onto every entry's name — the archive's own top directory.
func copyArchiveInto(ctx context.Context, eng Engine, containerID, p, root string, tw *tar.Writer) error {
	...
	for {
		...
		if root != "" {
			hdr.Name = root + "/" + strings.TrimPrefix(hdr.Name, "./")
		}
