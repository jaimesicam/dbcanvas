...
type mongoSecrets struct {
	...
	// The two MClusterAdmin accounts, when the node/frame asked for them
	// (MCACredentials). Empty when it did not — nothing is created, so nothing is
	// stored. The passwords are the panel's, not generated here.
	MCAUser             string `json:"mcaUser"`
	MCAPassword         string `json:"mcaPassword"`
	MCAReadOnlyUser     string `json:"mcaReadonlyUser"`
...
func (a *App) provisionMongoDBFrame(st Stack, frame designFrame, doc designDoc) {
	...
	// keyFile + PMM/PBM passwords are non-canvas secrets, still reused across
	// redeploys. The MClusterAdmin ones are neither: they are canvas fields on the
	// panel node, so they are re-read every time like the admin password.
	...
	// those accounts exist — neither can carry an OIDC or directory-auth block, so
	// there is nothing here for this one to collide with.
	mcaParams := mongoMCASetParams(frame.MCACredentials, false, false)
	...
	go func() {
		...
		// ---- MClusterAdmin: the accounts go on the config replica set AND on every
		// shard. mongos reads users from the config RS, which covers everything that
		// talks to the router — but the panel also connects straight to each shard
...
func (a *App) provisionMongoStandalone(st Stack, n designNode, doc designDoc) {
	...
	mcaAdminPW, mcaROPW := mcaPasswordsFor(doc, n.ID)
	mcaSecretsFor(&sec, n.MCACredentials, mcaAdminPW, mcaROPW)
	...
	go func() {
		...
		nn.Role = "standalone"
		if setParams == "" {
			// OIDC did not claim the block. LDAP/Kerberos writes its own at deploy
			// (dbauth.go) and that one already names both SCRAM mechanisms, so this
			// stays out of its way rather than producing a duplicate YAML key.
			setParams = mongoMCASetParams(n.MCACredentials, n.EnableOIDC, n.LdapAuth || n.KerberosAuth)
		}
		...
		// MClusterAdmin: the panel's account on the standalone.
		a.mongoEnsureMCAUsers(ctx, st, nn, sec, pr)
...
// ---------------------------------------------------- MClusterAdmin (madmin)
//
// MClusterAdmin is a MongoDB administration panel that a stack can run as a node of
// its own (see mclusteradmin.go). It authenticates as an ordinary MongoDB user, so a
// MongoDB node or cluster can be asked at design time to create the two accounts it
// uses — that is the MCACredentials field.
//
// Both are created, not one: which of them you want is a property of the SESSION,
// not of the database. A panel started read-only connects as madmin-ro and one that
// is not connects as madmin, and having both there means changing your mind is a
// redeploy of the panel rather than of the database under it.
//
// The role lists are upstream's, not ours, and they are worth keeping verbatim: they
// are least-privilege sets worked out per dashboard, and they deliberately avoid root
// and clusterAdmin so the panel cannot drop a database. Each role is here because
// some panel feature does not work without it —
//
//	clusterMonitor        monitoring, RS and sharding status, getLog, config.*
//	clusterManager        balancer, default read/write concern, replSetReconfig
//	hostManager           killOp, setParameter (flow control)
//	dbAdminAnyDatabase    drop index, profile command, dbStats/collStats
//	readAnyDatabase       explain on user collections, find on any DB
//	userAdminAnyDatabase  user and role management
//	read on local         oplog.rs, for the Oplog Stats dashboard
//
// — and madmin-ro is the same list with everything that writes removed, which is why
// it pairs with the panel's own --view-only flag: MongoDB refuses the write anyway,
...
// mcaDefaultPasswords are what the accounts get when the stack has no MClusterAdmin
// node to take them from, or its fields are left empty. Fixed and documented rather
// than generated, for the same reason MONGODB_ADMIN_PASSWORD is: this is a lab, the
// password has to be typeable into a panel by hand, and a per-deploy random one that
// nothing displays is a credential nobody can use.
func mcaDefaultPasswords() (admin, readOnly string) {
	return envOr("MCLUSTERADMIN_PASSWORD", "madmin_password"),
		envOr("MCLUSTERADMIN_RO_PASSWORD", "madmin_ro_password")
}

// mcaPasswordsFor resolves the two passwords a MongoDB should create its panel
// accounts with, from the stack's MClusterAdmin node.
//
// The stack's ONLY panel. There is no association line to ask (the panel takes
// none — the URI is typed into its own UI), so two panels with different passwords
// is genuinely ambiguous: it takes the documented defaults rather than picking one
// of them, and the panel whose fields say otherwise reports that the password did
// not work — a better failure than a database quietly trusting the wrong one.
func mcaPasswordsFor(doc designDoc, _ string) (admin, readOnly string) {
	admin, readOnly = mcaDefaultPasswords()

	var panels []designNode
	for _, n := range doc.Nodes {
	...
	if pick.MCAAdminPassword != "" {
		admin = pick.MCAAdminPassword
	}
	if pick.MCAReadOnlyPassword != "" {
		readOnly = pick.MCAReadOnlyPassword
	}
	return admin, readOnly
}

// mcaRolesJS is the role array for one of the two accounts, as mongosh JavaScript.
func mcaRolesJS(readOnly bool) string {
	if readOnly {
		return `[{role:"clusterMonitor",db:"admin"},{role:"readAnyDatabase",db:"admin"},{role:"read",db:"local"}]`
	}
	return `[{role:"clusterMonitor",db:"admin"},{role:"clusterManager",db:"admin"},` +
		`{role:"hostManager",db:"admin"},{role:"dbAdminAnyDatabase",db:"admin"},` +
		`{role:"readAnyDatabase",db:"admin"},{role:"userAdminAnyDatabase",db:"admin"},` +
		`{role:"read",db:"local"}]`
}

// mcaUserJS creates one account, or updates it in place when it is already there — a
// redeploy must not fail on its own previous run, and a password changed on the
// canvas has to reach the database.
//
// mechanisms is pinned to SCRAM-SHA-256 rather than left to the server's default of
// storing both SCRAM-SHA-1 and SCRAM-SHA-256 credentials. Two reasons: it is what the
// panel negotiates, so this is the credential that actually gets used; and an account
// that exists with only the other mechanism's credentials fails authentication with a
// message about SCRAM-SHA-256 that looks like a password problem and is not.
//
// Specifying mechanisms on updateUser requires the password to be set in the same
// command unless the new set is a subset of the old — which it is here, because the
// password is always supplied.
func mcaUserJS(user, pass, rolesJS string) string {
	return fmt.Sprintf(`var a=db.getSiblingDB("admin");
try{a.createUser({user:%q,pwd:%q,roles:%s,mechanisms:["SCRAM-SHA-256"]})}catch(e){if(/already exists/i.test(e.message)){a.updateUser(%q,{pwd:%q,roles:%s,mechanisms:["SCRAM-SHA-256"]})}else throw e}`,
		user, pass, rolesJS, user, pass, rolesJS)
}

// mcaUsersJS is both accounts in one mongosh run — one exec instead of two, and they
// are created together or not at all.
func mcaUsersJS(adminPW, readOnlyPW string) string {
	return mcaUserJS(mcaAdminUser, adminPW, mcaRolesJS(false)) + "\n" +
		mcaUserJS(mcaReadOnlyUser, readOnlyPW, mcaRolesJS(true))
}

// mcaSecretsFor records the accounts a MongoDB is about to create, so the panel node
// can build a connection string from them and the database's own panel can show
// them. Nothing is recorded when the box is not ticked: an empty MCAUser is what
// every reader treats as "this database has no panel accounts".
func mcaSecretsFor(sec *mongoSecrets, on bool, adminPW, readOnlyPW string) {
	if !on {
		return
	}
	sec.MCAUser, sec.MCAPassword = mcaAdminUser, adminPW
	sec.MCAReadOnlyUser, sec.MCAReadOnlyPassword = mcaReadOnlyUser, readOnlyPW
}

// mongoSCRAMSetParameter is a mongod.conf/mongos.conf setParameter block that names
// the SCRAM mechanisms explicitly.
//
// It is written when a MongoDB is asked for the MClusterAdmin accounts, and it is
// the half of that feature that actually makes the panel able to log in. The
// accounts are created with SCRAM-SHA-256 credentials, and a server whose
// authenticationMechanisms do not include SCRAM-SHA-256 then refuses them with an
// error that names the mechanism and reads like a wrong password. Naming both
// mechanisms rather than only SCRAM-SHA-256 keeps every account that already exists
// working — admin, PMM and PBM are created without a mechanisms list, so they carry
// credentials for both.
//
// mongod.conf can hold only ONE setParameter block, so this is used only where
// nothing else renders one: OIDC's own block (mongoOIDCSetParameter) already lists
// both SCRAM mechanisms, and the LDAP/Kerberos path appends its own at deploy time
// (dbauth.go) which also lists them. The callers check for those; a second
// setParameter key would be a duplicate YAML key and mongod would not start.
func mongoSCRAMSetParameter() string {
	return "setParameter:\n  authenticationMechanisms: SCRAM-SHA-1,SCRAM-SHA-256\n"
}

// mongoMCASetParams returns that block when this MongoDB is creating the panel
// accounts and nothing else is already rendering a setParameter block for it.
func mongoMCASetParams(on, oidc, dirAuth bool) string {
	if !on || oidc || dirAuth {
		return ""
	}
	return mongoSCRAMSetParameter()
}

// mcaShardedAccountHosts lists the members a sharded cluster's MClusterAdmin
// accounts have to be created on: the config replica set's primary, AND the primary
// of every shard.
//
// The config RS alone is not enough, which is what made a sharded cluster refuse the
// panel while a replica set accepted it. Two different things are true at once:
//
//   - Cluster-wide authentication goes through mongos, which reads users from the
//     config replica set. That is why mongoCreateAdmin puts the admin user there and
//     nowhere else, and it is enough for anything that talks to mongos.
//   - A shard replica set is also a replica set, with its own user database. The
//     panel connects DIRECTLY to shards for the per-shard views — replica-set status,
//     oplog, current ops, the cross-shard user sync — and prompts for credentials
//     when it does. An account that exists only on the config RS is not there, and
//     "could not log in to rs0" is exactly what you get.
//
// The shard primaries are also where mongoEnsurePMMUser and mongoEnsurePBMUser
// already put their own accounts, for the same reason, through the same script and
// its localhost-exception path — a shard that has no users yet gets an admin user
// created first, so this leaves each shard reachable as admin too.
func mcaShardedAccountHosts(config []designNode, shards map[int][]designNode, shardIdx []int) []designNode {
	if len(config) == 0 {
		return nil
	}
	hosts := []designNode{config[0]}
	for _, i := range shardIdx {
		if len(shards[i]) > 0 {
			hosts = append(hosts, shards[i][0])
		}
	}
	return hosts
}

// mongoEnsureMCAUsers creates/updates both MClusterAdmin accounts on one member that
// holds a user database: a standalone, a replica-set primary, a sharded cluster's
// config replica-set primary, or a shard's primary. Called once per such member —
// see mcaShardedAccountHosts for why a sharded cluster has several.
//
// Best-effort by design: it runs after the cluster is up, and a panel account that
// could not be created is a reason to say so in the log, not to fail a deployment
// that is otherwise serving.
func (a *App) mongoEnsureMCAUsers(ctx context.Context, st Stack, node designNode, sec mongoSecrets, pr *pxcProg) {
	if sec.MCAUser == "" {
		return
	}
	...
	js := mcaUsersJS(sec.MCAPassword, sec.MCAReadOnlyPassword)
	env := []string{"ADMIN_USER=" + sec.AdminUser, "ADMIN_PW=" + sec.AdminPassword, "USER_JS=" + js}
	if err := a.runStep(ctx, dep.ContainerID, mongoAdminEvalScript, env, pr.logln); err != nil {
		pr.logln("MClusterAdmin users not created: " + err.Error())
		return
	}
	pr.logln("MClusterAdmin users ready: " + sec.MCAUser + " (full) and " + sec.MCAReadOnlyUser + " (read-only), SCRAM-SHA-256")
}
...
func (a *App) mongoStartMongos(ctx context.Context, st Stack, mongos designNode, configDB, setParams string, pr *pxcProg) error {
	dep, _ := a.store.GetDeployment(st.ID, mongos.ID)
	if err := a.engCtx(ctx).CopyFile(ctx, dep.ContainerID, "/etc", "mongos.conf", 0o644, []byte(mongosConfYAML(configDB, setParams))); err != nil {
...
func mongoOIDCIssues(n designNode, keycloakIDs, keycloakSSL map[string]bool) []issue {
	...
	if !keycloakIDs[n.KeycloakNodeID] {
		return []issue{{Level: "error", Message: "PSMDB node " + n.Label + " has Keycloak OIDC enabled but is not linked to a Keycloak node — add a Keycloak node and select it"}}
	}
	if !keycloakSSL[n.KeycloakNodeID] {
		return []issue{{Level: "error", Message: "PSMDB node " + n.Label + " uses Keycloak OIDC, which requires an HTTPS issuer — enable \"Use Intranet CA SSL\" on the Keycloak node"}}
	}
	if n.LdapAuth && n.KerberosAuth {
		return []issue{{Level: "error", Message: "PSMDB node " + n.Label + " has LDAP, Kerberos and Keycloak OIDC enabled — MongoDB cannot combine Keycloak OIDC with directory authentication (each needs its own mongod.conf setParameter block); turn off Keycloak SSO, or turn off both LDAP and Kerberos"}}
	}
	if n.LdapAuth {
		return []issue{{Level: "error", Message: "PSMDB node " + n.Label + " has both LDAP and Keycloak OIDC enabled — MongoDB cannot use both (each needs its own mongod.conf setParameter block); turn one off"}}
	}
	if n.KerberosAuth {
		return []issue{{Level: "error", Message: "PSMDB node " + n.Label + " has both Kerberos and Keycloak OIDC enabled — MongoDB cannot use both (each needs its own mongod.conf setParameter block); turn one off"}}
	}
	return nil
}
...
// setParams is the same already-rendered setParameter block mongodConfYAML takes.
// A sharded cluster is administered through mongos, so the mechanism has to be
// advertised here as well as on the shards behind it — a router that will not
// negotiate SCRAM-SHA-256 refuses the panel no matter what the config servers say.
func mongosConfYAML(configDB, setParams string) string {
	var b strings.Builder
	fmt.Fprintf(&b, "systemLog:\n  destination: file\n  path: %s/mongos.log\n  logAppend: true\n", mongoLogDir)
	fmt.Fprintf(&b, "net:\n  port: %d\n  bindIpAll: true\n", mongoPort)
...
// mongoAdminEvalScript runs $USER_JS as the admin user. When those credentials do
// not work yet it first creates the admin user through the localhost exception —
// that is the sharded case, where a shard's own replica set has no admin user of
// its own and only the config RS does. Shared by the PMM and MClusterAdmin user
// steps, which differ only in the JavaScript they hand it.
const mongoAdminEvalScript = `set -e
...
mongosh --quiet --port 27017 -u "$ADMIN_USER" -p "$ADMIN_PW" --authenticationDatabase admin --eval "$USER_JS"`
