...
func aioIssues(n designNode, doc designDoc, exportReq map[int][]string, hostMemBytes int64) []issue {
	...
	if len(n.AIOInstances) == 0 {
		out = append(out, issue{Level: "warning", Message: "All-in-One node " + label + " has no features added — it will deploy as a bare host"})
		return out
	}
	...
	for _, in := range n.AIOInstances {
		...
		if !ok {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One node %s has an instance of unknown kind %q", label, in.Kind)})
			continue
		}
		iname := strings.TrimSpace(in.Name)
		if iname == "" {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One node %s has an unnamed %s instance", label, k.Label)})
			continue
		}
		...
		if why, blocked := aioUnsupportedModes[in.Kind+":"+strings.TrimSpace(in.ReplMode)]; blocked {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s: %s", iname, why)})
		}
		...
		if in.PMMNodeID != "" && !nodeOfTypeExists(doc, in.PMMNodeID, "pmm") {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s is monitored by a PMM node that is not on the canvas", iname)})
		}
		if in.OpenBaoNodeID != "" && !nodeOfTypeExists(doc, in.OpenBaoNodeID, "openbao") {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s references an OpenBao node that is not on the canvas", iname)})
		}
		if in.KeycloakNodeID != "" && !nodeOfTypeExists(doc, in.KeycloakNodeID, "keycloak") {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s references a Keycloak node that is not on the canvas", iname)})
		}
		if in.SeaweedFSNodeID != "" && !nodeOfTypeExists(doc, in.SeaweedFSNodeID, "seaweedfs") {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s references a SeaweedFS node that is not on the canvas", iname)})
		}
		if in.LdapAuth && in.LdapDirNodeID != "" &&
			...
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s authenticates against a directory node that is not on the canvas", iname)})
		}
		...
		if ref := in.OrchestratorRef; ref != "" {
			if !aioOrchSupported(in.Kind) {
				out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s (%s) cannot be monitored by Orchestrator — it manages async and semi-sync replication only", iname, k.Label)})
			} else if local, isLocal := strings.CutPrefix(ref, "inst:"); isLocal {
				if !instIDs[local] {
					out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s is monitored by an Orchestrator instance that no longer exists on this node", iname)})
				}
			} else if !nodeOfTypeExists(doc, ref, "orchestrator") {
				out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s is monitored by an Orchestrator node that is not on the canvas", iname)})
			}
		}
		...
		if k.Family == famProxy || k.Family == famHAProxy {
			if in.BackendInstance == "" {
				out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s (%s) has no backend selected — pick the instance it should front", iname, k.Label)})
			} else if !instIDs[in.BackendInstance] {
				out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s (%s) fronts an instance that no longer exists on this node", iname, k.Label)})
			} else if b := aioInstanceByID(n.AIOInstances, in.BackendInstance); b != nil && !aioProxyCanFront(in.Kind, b.Kind) {
				out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One instance %s (%s) cannot front a %s instance", iname, k.Label, aioKindByID[b.Kind].Label)})
			}
		}
	...
	for name, count := range names {
		if count > 1 {
			out = append(out, issue{Level: "error", Message: fmt.Sprintf("All-in-One node %s has %d instances named %q — names must be unique within the node", label, count, name)})
		}
	}
	...
	if orch > 1 {
		out = append(out, issue{Level: "warning", Message: fmt.Sprintf("All-in-One node %s has %d Orchestrator instances — one discovers every cluster in the stack already", label, orch)})
	}
	if daemons > 20 {
		out = append(out, issue{Level: "warning", Message: fmt.Sprintf("All-in-One node %s runs %d database daemons in one container — expect a slow deploy and high memory use", label, daemons)})
	}
