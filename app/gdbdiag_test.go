...
// execEngine is an Engine that only answers Exec, which is all gdbReadSource asks of
// one. Everything else is the embedded nil interface and panics if it is called —
// deliberately: a test that starts needing a container has stopped testing this.
type execEngine struct {
	Engine
	cmd  string // the shell command it was handed, for asserting what was run
	out  string
	code int
}

func (e *execEngine) Exec(_ context.Context, _ string, cmd []string, _ []string) (ExecResult, error) {
	e.cmd = cmd[len(cmd)-1]
	return ExecResult{Stdout: e.out, Code: e.code}, nil
}

// The source pane shows the WHOLE file: reading a crash means scrolling out of the
// frame's own function, and a window would end exactly where that starts. So this
// reads the file entire, numbers it from 1, and only the size cap bounds it.
func TestGDBReadSourceReturnsTheWholeFile(t *testing.T) {
	app := &App{}
	var body strings.Builder
	for i := 1; i <= 5000; i++ {
		fmt.Fprintf(&body, "line %d\n", i)
	}
	eng := &execEngine{out: body.String()}
	ctx := withEngine(context.Background(), eng)

	file := "/usr/src/debug/percona-server-8.0.30-22/storage/innobase/fts/fts0que.cc"
	lines, truncated, err := app.gdbReadSource(ctx, "cid", file)
	if err != nil {
		t.Fatal(err)
	}
	if truncated {
		t.Error("a 5,000-line file was reported as truncated")
	}
	if len(lines) != 5000 {
		t.Fatalf("got %d lines, want the whole 5,000", len(lines))
	}
	if lines[0] != "line 1" || lines[4999] != "line 5000" {
		t.Errorf("the file is not whole: first %q, last %q", lines[0], lines[4999])
	}
	// Read with head, not sed: no window, no line arithmetic on the node.
	if !strings.Contains(eng.cmd, "head -c") || strings.Contains(eng.cmd, "sed") {
		t.Errorf("the read is not a whole-file read: %q", eng.cmd)
	}
	if !strings.Contains(eng.cmd, file) {
		t.Errorf("the command does not name the file: %q", eng.cmd)
	}

	// A path from the crashed build that points outside the node's source trees is
	// refused before anything runs.
	if _, _, err := app.gdbReadSource(ctx, "cid", "/etc/shadow"); err == nil {
		t.Error("a path outside the source trees was read")
	}

	// Nothing there at all is the missing-debugsource case, and says so.
	empty := &execEngine{out: ""}
	if _, _, err := app.gdbReadSource(withEngine(context.Background(), empty), "cid", file); err == nil ||
		!strings.Contains(err.Error(), "debugsource") {
		t.Errorf("an empty read should name the missing package, got %v", err)
	}
}

// The cap exists because this crosses a websocket into a browser. When it bites, the
// pane has to be told — and must not be handed half of a line as if it were code.
func TestGDBReadSourceTruncatesOnWholeLines(t *testing.T) {
	app := &App{}
	var body strings.Builder
	for body.Len() <= gdbSourceMax+4096 {
		body.WriteString("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n")
	}
	// The engine returns what `head -c gdbSourceMax+1` would: one byte past the cap.
	eng := &execEngine{out: body.String()[:gdbSourceMax+1]}
	lines, truncated, err := app.gdbReadSource(withEngine(context.Background(), eng), "cid",
		"/usr/src/debug/percona-server-8.0.30-22/sql/sql_yacc.cc")
	if err != nil {
		t.Fatal(err)
	}
	if !truncated {
		t.Fatal("a file past the cap was not reported as truncated")
	}
	for i, ln := range lines {
		if len(ln) != 40 {
			t.Fatalf("line %d is %d characters — a partial line was kept", i+1, len(ln))
		}
	}
}
