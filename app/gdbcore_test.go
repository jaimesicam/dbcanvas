...
// The recipe is what somebody pastes into a terminal, or into a ticket, when the page has run out
// of answers. Two things make it worth generating rather than writing down: it has to name THIS
// node's paths, and it has to produce the same backtrace the page did.
func TestGDBRecipeReproducesTheSession(t *testing.T) {
	solib := "/sysroot:/sysroot/lib64"
	src := []string{"/usr/src/debug/percona-server-8.0.30-22.1.el9.x86_64"}
	got := gdbRecipe("dbcanvas-12-linuxclient-01", "/sysroot/mysqld", "/coredumps/core.1234",
		gdbSysrootMount, solib, src)
	...
	// Everything that decides what the backtrace SAYS is kept. past-main/past-entry is the one
	// people never think to set, and it is the difference between seeing the bottom of a
	// stack-exhaustion core and having it cut off.
	for _, kept := range []string{"set backtrace past-main on", "set backtrace past-entry on"} {
		if !strings.Contains(gdbStartScript, kept) {
			t.Fatalf("gdbStartScript no longer sets %q — the recipe is copying a session that does not exist", kept)
		}
		if !strings.Contains(got, kept) {
			t.Errorf("the recipe drops %q, so it would print a different stack from the page", kept)
		}
	}

	// And the parts that exist only because a machine is on the other end are dropped: a person
	// gets a prompt, not the machine interface, and should be asked before something is discarded.
	for _, dropped := range []string{"--interpreter", "mi3", "stty", "set confirm off"} {
		if strings.Contains(got, dropped) {
			t.Errorf("the recipe carries %q, which is only for the machine interface:\n%s", dropped, got)
		}
	}

	// It ends at a prompt with the stack already printed, which is what the command in this
	// file's own header does and the reason most people open gdb at all.
	if !strings.Contains(got, `-ex "thread apply all bt"`) {
		t.Error("the recipe does not print the stack")
	}

	// No container name (a target resolved before the deployment carried one) still yields a
	// usable line to run inside the node, rather than "docker exec -it  gdb".
	bare := gdbRecipe("", "/sysroot/mysqld", "/coredumps/core.1234", gdbSysrootMount, solib, nil)
	if strings.Contains(bare, "docker exec") || !strings.HasPrefix(bare, "gdb -nx") {
		t.Errorf("without a container the recipe should be the gdb line alone:\n%s", bare)
	}
	// Nothing to run against is no recipe at all, rather than a line with a hole in it.
	if gdbRecipe("c", "", "/coredumps/core.1234", gdbSysrootMount, solib, nil) != "" {
		t.Error("a target with no binary produced a recipe")
	}
	if gdbRecipe("c", "/sysroot/mysqld", "", gdbSysrootMount, solib, nil) != "" {
		t.Error("a session with no core produced a recipe")
	}
}

// A path only gets quotes when it needs them: a line full of quotes nobody needs is a line people
// edit before pasting, and an edited line is no longer the one that was run.
func TestGDBShellArg(t *testing.T) {
	for _, plain := range []string{"/coredumps/core.1234", "/sysroot/mysqld", "dbcanvas-12-linuxclient-01"} {
		if got := gdbShellArg(plain); got != plain {
			t.Errorf("gdbShellArg(%q) = %q, want it untouched", plain, got)
		}
	}
	for _, needs := range []string{"/coredumps/core dump", "/coredumps/$(id)", "/core;rm -rf /", ""} {
		if got := gdbShellArg(needs); !strings.HasPrefix(got, "'") {
			t.Errorf("gdbShellArg(%q) = %q, want it quoted", needs, got)
		}
	}
}
