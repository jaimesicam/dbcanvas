...
func stockSimEngineAndIssues(doc designDoc, n designNode) (string, []issue) {
	...
	case "aio":
		...
		if !ok {
			return "", []issue{{Level: "error", Message: "Stock Market Sim node " + n.Label +
		...
		if engine == "" {
			return "", []issue{{Level: "error", Message: fmt.Sprintf(
...
func stockSimSizeIssues(n designNode, engine string) []issue {
	...
	if size > 0 && size < 1<<30 {
		return []issue{{Level: "info", Message: fmt.Sprintf(
...
func stockSimIssues(n designNode) []issue {
	...
	if strings.TrimSpace(n.SSHost) == "" && strings.TrimSpace(n.SSDSN) == "" {
		out = append(out, issue{Level: "error", Message: "Stock Market Sim node " + n.Label + " is set to a manual connection but has no host — enter one, or paste a full connection string under Advanced"})
	}
	if n.SSPort < 0 || n.SSPort > 65535 {
		out = append(out, issue{Level: "error", Message: "Stock Market Sim node " + n.Label + " has an invalid port — leave it blank for the engine default"})
	}
	...
	out = append(out, issue{Level: "info", Message: "Stock Market Sim node " + n.Label + " connects to a database outside this stack — dbcanvas cannot verify it before deploying, so use Test connection on the node to check it now"})
	return out
}
