...
func buildAPIRoutes() []apiRoute {
	return []apiRoute{
		...
		{Method: "GET", Path: "/api/system/settings", Group: gPrefs, Handler: m((*App).handleGetSystemSettings),
			Summary: "Instance-wide settings: the file-upload ceiling, whether SSH tunnelling is configured, and whether experimental features are shown."},
		...
		// --- node images --------------------------------------------------------
		{Method: "GET", Path: "/api/images", Group: gImages, Handler: m((*App).handleListExtraImages),
			Summary: "The images a node needs beyond the operating-system bases, each with whether it is present and any build in flight."},
		{Method: "POST", Path: "/api/images/{id}/build", Group: gImages, Auth: authAdmin, Handler: m((*App).handleBuildExtraImage),
			Summary: "Build one of those images on the Docker daemon DBCanvas already uses. Returns at once; poll GET /api/images for progress."},
		...
		// --- MongoDB node downloads ---------------------------------------------
		{Method: "GET", Path: "/api/stacks/{id}/nodes/{nid}/mongo/diagnostic", Group: gNodes, Media: mediaDownload, Handler: m((*App).handleMongoDiagDownload),
			Summary: "Download a MongoDB node's diagnostic.data (FTDC) with its mongod.log (mongos.log on a router), as a `.tar.gz` holding one `<hostname>_log_diagnostic_data` directory."},
		...
		// --- Stalk Summary ------------------------------------------------------
		// pt-k8s-debug-collector on a K3D cluster. Keyed to the frame, not a node:
		// the capture is of the whole cluster, and the k3s node it is taken
		// through is an implementation detail.
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/collect", Group: gCaptures, Handler: m((*App).handleK8sCollectStatus),
			Summary: "Whether a pt-k8s-debug-collector capture is running on this Kubernetes cluster."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/collect", Group: gCaptures, Handler: m((*App).handleK8sCollectStart),
			Summary: "Start a pt-k8s-debug-collector capture of this Kubernetes cluster."},
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/collect/dumps", Group: gCaptures, Handler: m((*App).handleK8sDumps),
			Summary: "The kept cluster-dump captures for this Kubernetes cluster."},
		{Method: "GET", Path: "/api/k8sdumps/{did}/download", Group: gCaptures, Media: mediaDownload, Handler: m((*App).handleK8sDumpDownload),
			Summary: "Download a kept cluster-dump archive."},
		{Method: "DELETE", Path: "/api/k8sdumps/{did}", Group: gCaptures, Handler: m((*App).handleK8sDumpDelete),
			Summary: "Delete a kept cluster-dump archive and its file on disk."},

		// ---- Operator Summary ----
		{Method: "GET", Path: "/api/opsummary/dumps", Group: gOpSum, Handler: m((*App).handleOpSummaryDumps),
			Summary: "Every cluster-dump you have kept, newest first — they outlive the clusters they came from."},
		{Method: "POST", Path: "/api/opsummary/upload", Group: gOpSum, Media: mediaMultipart, Handler: m((*App).handleOpSummaryUpload),
			Summary: "Upload a pt-k8s-debug-collector cluster-dump and distil it into workload health, custom-resource status and operator errors."},
		{Method: "POST", Path: "/api/opsummary/dumps/{did}", Group: gOpSum, ReadOnly: true, Handler: m((*App).handleOpSummaryFromDump),
			Summary: "Analyse a cluster-dump already kept on this installation."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/opsummary", Group: gOpSum, ReadOnly: true, Handler: m((*App).handleOpSummaryNode),
			Summary: "Capture and analyse a Kubernetes cluster's cluster-dump in one request."},
		...
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/logicalreplicas", Group: gK3D, Handler: m((*App).handleK3DLogicalReplicas),
			Summary: "The cluster's logical replicas: spec, status, Service address, and the documented preflight for adding one. Percona Operator for PostgreSQL 3.1.0+."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/logicalreplicas", Group: gK3D, Handler: m((*App).handleK3DLogicalReplicaAdd),
			Summary: "Add a logical replica to a running cluster (spec.logicalReplicas)."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/logicalreplicas/remove", Group: gK3D, Handler: m((*App).handleK3DLogicalReplicaRemove),
			Summary: "Remove a logical replica. The operator always deletes its volume with it."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/logicalreplicas/reseed", Group: gK3D, Handler: m((*App).handleK3DLogicalReplicaReseed),
			Summary: "Reseed a logical replica: remove it, wait for the operator to drop it from status, add it back. The operator does not rebuild one in place."},
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/logicalreplicas/bootstraplog", Group: gK3D, Handler: m((*App).handleK3DLogicalReplicaBootstrapLog),
			Summary: "The bootstrap Job's log for one logical replica — where a replica stuck in `bootstrapping` explains itself."},
		...
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/backups", Group: gK3D, Handler: m((*App).handleK3DBackups),
			Summary: "The cluster's backup and restore custom resources, with the object store they go to."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/backups", Group: gK3D, Handler: m((*App).handleK3DBackupCreate),
			Summary: "Take a backup: apply a Backup custom resource and archive its manifest in the operator's deploy/backup. `dryRun` returns the manifest without applying it."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/backups/delete", Group: gK3D, Handler: m((*App).handleK3DBackupDelete),
			Summary: "Delete a backup object. `data` also clears what it wrote, by setting the percona.com/delete-backup finalizer first."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/restores", Group: gK3D, Handler: m((*App).handleK3DRestore),
			Summary: "Restore a backup onto the cluster: apply a Restore custom resource and archive its manifest. The cluster stops serving while it runs."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/restores/delete", Group: gK3D, Handler: m((*App).handleK3DRestoreDelete),
			Summary: "Delete a restore object. The record only — the data it restored is not affected."},
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/backups/manifests", Group: gK3D, Handler: m((*App).handleK3DBackupManifests),
			Summary: "The manifests this panel has applied, archived in the operator's deploy/backup. `name` returns one file's contents."},
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/bucket", Group: gK3D, Handler: m((*App).handleK3DBucket),
			Summary: "One prefix of the cluster's backup bucket, listed by `aws s3api` in a toolbox pod on the cluster."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/bucket/delete", Group: gK3D, Handler: m((*App).handleK3DBucketDelete),
			Summary: "Delete an object or a whole prefix from the backup bucket. `dryRun` reports what would go without deleting it."},
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/bucket/download", Group: gK3D, Media: mediaDownload, Handler: m((*App).handleK3DBucketDownload),
			Summary: "Stream one object out of the backup bucket, up to 64 MiB."},
		{Method: "GET", Path: "/api/stacks/{id}/frames/{fid}/k3d/bucket/toolbox", Group: gK3D, Handler: m((*App).handleK3DBucketToolbox),
			Summary: "Whether the bucket toolbox pod is running, and what image it runs."},
		{Method: "POST", Path: "/api/stacks/{id}/frames/{fid}/k3d/bucket/toolbox", Group: gK3D, Handler: m((*App).handleK3DBucketToolbox),
			Summary: "Start or stop the bucket toolbox pod (`action`: start | stop)."},
		...
		{Method: "GET", Path: "/api/cli/help", Group: gMeta, Handler: m((*App).handleCLIHelp),
			Summary: "dbcanvas-cli's own `--help`, from the binary this image ships — every command with what it does."},
