...
func TestNotesNewerThan(t *testing.T) {
	...
	// A reader on a release older than anything in the catalogue has all of it to read. The
	// version to compare against is constructed rather than taken from the list: the catalogue
	// legitimately holds a single release, and then "older than the oldest note" is not in it.
	oldest := whatsNewNotes[len(whatsNewNotes)-1].Version
	if compareVersions(oldest, "0.0.1") <= 0 {
		t.Fatalf("the oldest note is %s; this test needs something below it to stand on", oldest)
	}
	rest := notesNewerThan("0.0.1")
	if len(rest) != len(whatsNewNotes) {
		t.Fatalf("a reader on 0.0.1 gets %d notes, want all %d", len(rest), len(whatsNewNotes))
	}
	for _, n := range rest {
		if compareVersions(n.Version, "0.0.1") <= 0 {
