...
**Take the session with you.** Once a core is open, **Run this in your own terminal** (under the
verdict) holds the command line that reproduces it — copy it with one click, into a shell or into a
ticket:
...
docker exec -it dbcanvas-12-linuxclient-01 gdb -nx \
  ...
  -ex "directory /usr/src/debug/percona-server-8.0.30-22.1.el9.x86_64" \
  -ex "thread apply all bt" \
  /sysroot/mysqld /coredumps/core.1234
```

It is generated, not a template: the container, the binary, the core file and — the part nobody can
write from memory — the `solib-search-path` DBCanvas resolved by walking the mounted library
directory when it opened this core. It is the same command the page itself runs, minus the machine
interface (`--interpreter=mi3`) and `set confirm off`, which is right for a program driving gdb and
wrong for a person. Everything that decides what the backtrace *says* is kept, `backtrace
...
- **The source, highlighted** — `*-debugsource` installs to exactly the path the debug
  information records, so the panel shows the crashing line itself, not just its coordinates, with
  the rest of the file above and below it to scroll through — and this held even for a
  dynamically-loaded plugin's own `.cc` file (`audit_log.cc`), which `ldd` would never have found
  on its own.
