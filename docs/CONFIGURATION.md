...
| `SSH_FORWARDING_HOST` | *(empty)* | Where this installation is reachable over SSH, as `IP[:port]` (port defaults to 22; `user@IP[:port]` also fills in the login). Empty = off. Set it on a **server** install: right-clicking a running node then offers **Copy SSH tunnel command**, the `ssh -L` line that forwards every port that node publishes to the same port on your own machine — so a PMM console or a database port bound to the server's loopback becomes reachable from your browser and your local client. |
