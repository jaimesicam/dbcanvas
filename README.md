...
**Benchmark** for workloads, a **Packet Inspector** that decodes MySQL and PostgreSQL traffic
off the wire, a
...
### Packet Inspector
Capture traffic on a provisioned **MySQL** or **PostgreSQL** node with `tcpdump` and read it
back as decoded protocol: statements, responses, response times, prepared statements,
result-set shapes, the replication stream — and the network problems underneath
(**retransmissions**, **gaps**, **duplicate ACKs**, **zero windows**, **resets**), plus the
operational errors that explain an outage: MySQL's `1205` lock-wait timeouts and `1047`
wsrep-not-ready, PostgreSQL's `40P01` deadlocks, `53300` too-many-connections, `25006` writes
that reached a standby, and `57P01` shutdowns. A cluster member's own traffic is captured too
— Galera's 4567/4568/4444 for PXC, **Patroni's REST API and etcd** for a Patroni member,
because a failover is decided in etcd rather than in PostgreSQL. Replication lag is read
straight off the wire from both ends' **LSNs**, with no access to either server. The
**Traffic Timeline** buckets density server-side and takes a range by drag, by packet number,
by time offset, by zoom/pan or by preset, so a 400k-packet capture stays responsive.
Encrypted sessions are marked rather than guessed at: sizes, timing and TCP behaviour stay
accurate and the statements are reported as unavailable. Uploaded pcaps decode the same way —
the protocol is sniffed from the bytes — and can carry the server's log alongside for
correlation. See [`docs/PACKET_INSPECTOR.md`](docs/PACKET_INSPECTOR.md).
